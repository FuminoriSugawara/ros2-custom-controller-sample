        ...
        const rclcpp::Time &time, const rclcpp::Duration &period)
    {
        ...
        switch (controller_state)
        {
            case ControllerState::INITIALIZING:
            {
                ...
                // ジョインが初期化されたかチェック
                bool all_joints_initialized = true;
                for (size_t i = 0; i < joint_configs_.size(); ++i)
                {
                    if (joint_configs_[i].is_target && !joint_configs_[i].current_status.bits.enable)
                    {
                        all_joints_initialized = false;
                        break;
                    }
                    
                }

                // すべてのジョイントが初期化されたら INITIAL_MODE_SETTING に移行
                if (all_joints_initialized)
                {
                    controller_state_.writeFromNonRT(ControllerState::INITIAL_MODE_SETTING);
                    RCLCPP_INFO(get_node()->get_logger(), "INITIAL_MODE_SETTING");
                }
                break;
            }
